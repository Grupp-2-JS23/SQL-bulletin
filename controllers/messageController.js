...
//asynchronous function to handle POST request to create message
const createMessage = async (req, res) => {
  const { message, sender, channelIds } = req.body;
  try {
    const validSubscriber = await messagesService.isUserSubscriber(
      sender,
      channelIds
    );
    if (!validSubscriber) {
      res.status(404).json({
        success: false,
        message: `User not subscriber on one or more channels `,
      });
      return;
    }
...
//asynchronous function to handle GET request to get messages
